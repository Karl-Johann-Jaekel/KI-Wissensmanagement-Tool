...
- [x] `docs/adr/` mit 14 ADRs, `plan.md`, `CLAUDE.md` im Repo
- [x] Tests grün (`pytest`, 158; im CI 157 + 1 übersprungen, weil `test_embed` das vorgeladene Modell braucht), Frontend-Build und `oxlint` ohne Befund, Browser-E2E über 23 Schritte
...
| Nachgelegt — Audit | Fünf Befunde eines externen Audits abgearbeitet. **Folgefragen:** jede kurze Frage bekam die vorige vorangestellt, auch für Volltext- und Normverweis-Suche; „Ab wann gilt …?" suchte nach „Artikel 50". Jetzt nur noch die Vektorsuche und nur bei echter Anapher, Regressionstest scheitert am alten Code. Die Doku hatte das als „bekannte Grenze" geführt — falsche Diagnose, korrigiert. **Prompt-Injection:** nachgewiesen (0/6); die empfohlene `<passage>`-Härtung gemessen schädlich und verworfen, stattdessen Antworten ohne Beleg im Code markiert, täuschende Antworten 6/6 → 3/6 (ADR-14). **Retrieval-Evaluation:** 17+5 Fragen, RRF hit@8 94 % gegen 71 % der besten Einzelliste, ein offener Fehlschlag (README). **Ingest** auf ein Dokument gleichzeitig begrenzt, Datenschutzhinweis auf Zugangsseite und im Upload-Dialog. **Hygiene:** Produktions-DB vor den Änderungen gesichert (`pg_dump`, 1,6 MB). **Nachtrag:** Eine unbelegte Antwort behält ihre Warnung auch als Notiz; dass die Normverweis-Liste je Instanz anders rankt (zufällige Quellen-ID bei gleichrangigen Überschriften), ist im README erklärt statt kurz vor der Abgabe umgebaut. 158 Backend-Tests, Browser-E2E 23 Schritte |
| Nachgelegt — Studio | Dritte Spalte ist jetzt das Studio: Briefing, FAQ und Themenkarte als Kacheln, darunter die Notizen. Die FAQ kostete eine Fragenquelle statt eines neuen Generierungspfads (ADR-13). Die Themenkarte zeichnet die 20 Kernthemen der Quellen-Guides als Karte — **ohne jede LLM-Anfrage**, weil die Daten schon in der Datenbank liegen; ein Klick auf ein Thema stellt die Frage im Chat. 130 Backend-Tests, Browser-E2E 22 Schritte grün |
